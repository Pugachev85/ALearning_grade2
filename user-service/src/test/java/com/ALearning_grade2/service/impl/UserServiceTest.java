...
class UserServiceTest {
    ...
    @Mock
    private KafkaTemplate<String, Object> kafkaTemplate; // ДОБАВЛЕНО: Mock для KafkaTemplate
    ...
    void createUserWithValidDataShouldCreateUser() {
        ...
        verify(kafkaTemplate, times(1)).send(anyString(), any()); // ДОБАВЛЕНО: Проверка вызова Kafka
    }
    ...
    void createUserWithExistingEmailShouldThrowException() {
        ...
        verify(kafkaTemplate, never()).send(anyString(), any()); // ДОБАВЛЕНО: Проверка, что Kafka не вызывается
    }
    ...
    void deleteUserWithExistingIdShouldReturnTrue() {
        // ДОБАВЛЕНО: Мок для поиска пользователя перед удалением
        when(userRepository.existsById(1L)).thenReturn(true);
        when(userRepository.findById(1L)).thenReturn(Optional.of(userEntity)); // ДОБАВЛЕНО: Для получения email

        boolean result = userService.deleteUser(1L);

        assertTrue(result);
        verify(userRepository, times(1)).deleteById(1L);
        verify(userRepository, times(1)).findById(1L); // ДОБАВЛЕНО: Проверка вызова findById
        verify(kafkaTemplate, times(1)).send(anyString(), any()); // ДОБАВЛЕНО: Проверка вызова Kafka
    }
    ...
    void deleteUserWithNonExistingIdShouldReturnFalse() {
        ...
        verify(userRepository, never()).findById(any()); // ДОБАВЛЕНО: Проверка, что findById не вызывается
        verify(kafkaTemplate, never()).send(anyString(), any()); // ДОБАВЛЕНО: Проверка, что Kafka не вызывается
    }
}
